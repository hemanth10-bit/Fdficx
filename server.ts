...
app.post("/api/text-query", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      ...
      severity: "low",
      image_needed: false,
      customer_reply: "I apologize, but I am experiencing temporary system issues. Let me log a help ticket for you, and a support representative will follow up."
    });
  }
});
...
app.post("/api/image-query", async (req, res) => {
  ...
  try {
    ...
    console.error("Gemini image support error:", err);
    res.status(500).json({
      visible_observation: "Unable to process image.",
      issue_type: "general",
      severity: "low",
      customer_reply: "I apologize, but I am having trouble viewing your image due to a system error. Let me forward this directly to our quality assurance team to assist you further."
    });
  }
});
...
async function startServer() {
  ...
  // Only run standard express app.listen if NOT in a Vercel/serverless context
